...
class Level:
    def __init__(self, level_number):
        ...
        if level_number == 1:
            self.create_shape(1, 1, 5, 5)
            self.create_shape(5, 10, 15, 11)

        if level_number == 2:
            self.create_shape(1, 1, 30, 2)
            self.create_shape(1, 10, 30, 11)
            self.create_shape(1, 19, 30, 20)
...
class GameField:
    def __init__(self, level):
        self.cells_content = level.level_cells_content
    ...
    def move(self):
        ...
        if self.direction == "LEFT":
            ...
            if is_border_reached:
                self.last_x = self.x
                self.last_y = self.y
                self.x = windowWidth//cellEdgeLength - 1
            elif right_tile_content == "ROCK":
                self.last_x = self.x
                self.last_y = self.y
                self.x -= 1
        ...
        if self.direction == "UP":
            ...
            if is_border_reached:
                self.last_x = self.x
                self.last_y = self.y
                self.y = windowHeight//cellEdgeLength - 1
            elif right_tile_content == "ROCK":
                self.last_x = self.x
                self.last_y = self.y
                self.y -= 1
...
if __name__ == "__main__":
    ...
    FPS = 60
    isRunning = True
    levelNumber = random.randint(1, 3)
    ...
    currentRock = Rock(random.randint(0, 47), random.randint(0, 20))
    ...
    while isRunning:
        ...
        print(timeSinceGameStart, snakeMoveSpeed)
        if isSnakeAlive:
